...
function AboutPage() {
  return (
    <main className="w-full min-h-screen bg-[var(--bg-hero)]">
      <div className="w-full max-w-5xl mx-auto">
        ...
        <div id="team" className="w-full grid grid-cols-2 md:grid-cols-3 my-12 gap-3 gap-y-10">
          {team.map((profile, index) => (
            <PersonProfile key={index} {...profile} />
          ))}
        </div>
        <h1 className="head-text mt-20">Advisors</h1>
        <div id="team" className="w-full grid grid-cols-2 md:grid-cols-3 my-12">
